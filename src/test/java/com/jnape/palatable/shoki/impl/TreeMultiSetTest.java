...
public class TreeMultiSetTest {
    ...
    public void sizeInfo() {
        assertEquals(finite(zero()), EMPTY.sizeInfo().getOrCompute());
        assertEquals(finite(one()), EMPTY.inc("foo", one()).sizeInfo().getOrCompute());
        assertEquals(finite(abs(2)), EMPTY.inc("foo", one()).inc("foo", one()).sizeInfo().getOrCompute());
        assertEquals(finite(abs(12)),
                     EMPTY.inc("foo", one()).inc("bar", atLeastOne(10)).inc("foo", one()).sizeInfo().getOrCompute());
    }
